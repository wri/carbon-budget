...
for year in range(2006, 2007):
    ...
    with open('vrt_files.txt', 'w') as vrt_files:
        ...
        for tif in vrt_tifs:
            vrt_files.write(tif + "\n")
    cmd = ['gdalbuildvrt', '-input_file_list', 'vrt_files.txt', '-tap', '-tr', '463.3127165', '463.3127165',  vrt_name]
    subprocess.check_call(cmd)
    
    # build new vrt and virtually project it
    vrt_wgs84 = 'global_vrt_{}_wgs84.vrt'.format(year)
    cmd = ['gdalwarp', '-of', 'VRT', '-t_srs', "EPSG:4326", 'global_vrt_2006.vrt', vrt_wgs84]
    subprocess.check_call(cmd)
    ...
    if __name__ == '__main__':
        ...
        pool.map(main.clip_year_tiles, tile_year_list)
