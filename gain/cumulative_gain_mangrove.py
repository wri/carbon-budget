...
def cumulative_gain(tile_id):
    ...
    accum_outfilename = '{0}_{1}.tif'.format(tile_id, cn.pattern_cumul_gain_AGC_mangrove)
    ...
    utilities.upload_final(cn.cumul_gain_AGC_mangrove_dir, tile_id, cn.pattern_cumul_gain_AGC_mangrove)

    print "  Reading input files and calculating cumulative belowground gain for mangrove tile {}".format(tile_id)
    accum_calc = '--calc=A*B*{}'.format(cn.biomass_to_c)
    accum_outfilename = '{0}_{1}.tif'.format(tile_id, cn.pattern_cumul_gain_BGC_mangrove)
    accum_outfilearg = '--outfile={}'.format(accum_outfilename)
    cmd = ['gdal_calc.py', '-A', gain_rate_BGB, '-B', gain_year_count, accum_calc, accum_outfilearg, '--NoDataValue=0', '--overwrite', '--co', 'COMPRESS=LZW']
    subprocess.check_call(cmd)

    utilities.upload_final(cn.cumul_gain_BGC_mangrove_dir, tile_id, cn.pattern_cumul_gain_BGC_mangrove)
