...
gross carbon removals by forests, and the difference between them 
(net flux), all between 2001 and 2019. 
Gross emissions includes CO2, NH4, and N20 and all carbon pools, and gross removals includes aboveground and belowground biomass carbon. 
...
manageable size), and 0.04 x 0.04 degrees (approximately 4 x 4 km at the equator) as global rasters. The former output is used for 
...
like in publications and presentations. 
The 30-m resolution output is in megagrams CO2e/ha 2001-2019 (i.e. densities), 
while the 4-km resolution output is in Mg CO2e/pixel 2001-2019 (in order to show absolute values).
...
and cannot be assigned specific years.
...
#### Running the model
There are two ways to run the model: as a series of individual scripts and from a master script, which run the individual scripts sequentially.
...
and often the number of processors being used is 1/2 or a 1/3 of the actual number available.
...
| `no_primary_gain` | Primary forests and IFLs are assumed to not have any removals| Global | `forest_age_category_IPCC.py` |
