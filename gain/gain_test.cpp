		...
		if (tcd_data[x] > 0) // on TCD > 0
		{
			if (plant_gr_data[x] > 0)   // plantations
			{
				...
				if (loss_data[x] > 0) // plantations, loss
				{
					...
					{
                                                outdata1 = (plant_gr_data[x] * (loss_data[x]-1)) + ((15-(loss_data[x]+1)/2) * plant_gr_data[x]);					}
					
					else // tcd, plantation, loss, no gain
					{
						outdata1 = (plant_gr_data[x]) * (loss_data[x] -1);
					}
				}
				else // plantation, no loss
				{
					...
					{
						outdata1 = plant_gr_data[x] * 7.5;
						
					}
					else // plantations, no loss, no gain
					{
						outdata1 = plant_gr_data[x] * 15;
					}
				}
			}
			else // no plantations
			{
				if (loss_data[x] > 0) // no plantations, loss
				{
					...
					{
						outdata1 = ((old_gr_data[x] + young_gr_data[x])/2) * (loss_data[x] -1) + ((((15-(loss_data[x]+1)))/2) * young_gr_data[x]);
					}
					else  // no plantations, loss, no gain
					{
						outdata1 = ((old_gr_data[x] + young_gr_data[x]) /2) * loss_data[x] -1;
					}
				}
				else // no plantations, no loss
				{
					...
					{
						outdata1 = ((old_gr_data[x] + young_gr_data[x]) /2) * 7.5;
					}
					else // no plantations, no loss, no gain
					{
						outdata1 = old_gr_data[x] * 15;
					}
				}
			}
		}
