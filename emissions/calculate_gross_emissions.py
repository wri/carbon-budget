...
def calc_emissions(tile_id, pools, sensit_type, folder):
    ...
    if (pools == 'soil_only') & (sensit_type == 'std'):
        emissions_tiles_cmd = ['/usr/local/app/emissions/cpp_util/calc_gross_emissions_soil_only.exe', tile_id, folder]

    elif (pools == 'biomass_soil') & (sensit_type in ['convert_to_grassland', 'no_shifting_ag']):
        emissions_tiles_cmd = ['/usr/local/app/emissions/cpp_util/calc_gross_emissions_{}.exe'.format(sensit_type), tile_id, folder]
    ...
    # This C++ script has an extra argument that names the input carbon pools and output emissions correctly
        ...
        emissions_tiles_cmd = ['/usr/local/app/emissions/cpp_util/calc_gross_emissions_generic.exe', tile_id, sensit_type, folder]
