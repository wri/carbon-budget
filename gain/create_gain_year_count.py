...
def create_gain_year_count(tile_id):
    ...
    no_change_calc = '--calc=(A==0)*(B==0)*15'
    ...
    cmd = ['gdal_calc.py', '-A', loss, '-B', gain, no_change_calc, no_change_outfile2, '--NoDataValue=0', '--overwrite', '--co', 'COMPRESS=LZW']
    subprocess.check_call(cmd)
    ...
    # cmd = ['gdal_merge.py', '-o', age_outfile, loss_outfile1, gain_outfile1, no_change_outfile1, loss_and_gain_outfile1, '-co', 'COMPRESS=LZW', '-n', '0', '-a_nodata', '0']
    # subprocess.check_call(cmd)
