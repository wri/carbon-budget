...
country_continent_growthrate_csv = r'country_continent_ecozone.csv'

country_continent_df = pd.read_csv(country_continent_growthrate_csv)

# continent = country_continent_dict[<country id>][<continent>]
country_continent_dict = country_continent_df.set_index('country_ecozone').T.to_dict()


dict = {'ATTRIBUTE': 81.0, 'old': 2.086098464, 'continent_int': 1.0, 'eco_zone': 20.0, 'young': 6.4840487479999993}

with open('testcpp.txt', 'w') as text:
    for key, value in country_continent_dict.iteritems():
        country = key.split("_")[0]
        eco_zone = key.split("_")[1]
        old = value['old']
        young = value['young']

        string = "\nelse if (country == {0} && ecozone == {1}){2}\nyoung = {3};\nold = {4};\n{5}".format(country, eco_zone, '{', young, old, '}' )
