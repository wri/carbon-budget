    ...
    // Everything from here down analyzes one pixel at a time
	{
		...
		if (loss_data[x] > 0 && agc_data[x] > 0)
        {
			...
			if (drivermodel_data[x] == 1)
			{
				...
				minsoil_CO2_only = average_annual_minsoil_c_loss * (model_years-loss_data[x]) * C_to_CO2;
                minsoil_N2O_only = average_annual_minsoil_c_loss * (1/C_N_ratio) * N_mineralization_EF * N2O-N_to_N2O * N2O_equiv;  // Note didn't multiply by 1000 to keep in t instead of kg [IPCC 2019, V4, Ch. 11, Equations 11.8 (F_som) and 11.1 (total emissions)]
                //todo: split minsoil into co2 and n2o only
                //todo: do I need to multiply n2o by (model_years-loss_data[x])?
			...
			else if (drivermodel_data[x] == 3)
			{
				...
				if (peat_data[x] > 0) // shifting cultivation, peat
				{
					if (burn_data[x] > 0) // shifting cultivation, peat, burned
					{
						...
						{
						    outdata_shifting_cultivation_CO2only = peat_drain_total_CO2_only + peatburn_CO2_only;
