...
def mp_create_soil_C(tile_id_list):
    ...
    :return: sets of tiles with soil organic C density for mangroves only, SoilGrids250, mangroves overlaid on SoilGrids250,
    and standard deviation in soil organic C
    """
    ...
    # else:
    #     if cn.count == 96:
    #         processes = 62   # 45 processors = 420 GB peak; 55 = 500 GB peak; 62 = XXX GB peak
    #     else:
    #         processes = int(cn.count/2)
    ...
    cmd = ['wget', '--recursive', '-nH', '--cut-dirs=6', '--no-parent', '--reject', 'index.html*',
                   ...
                   '--accept', '*.tif', f'{cn.CI5_mineral_soil_C_url}']
    uu.log_subprocess_output_full(cmd)
    ...
    cmd = ['wget', '--recursive', '-nH', '--cut-dirs=6', '--no-parent', '--reject', 'index.html*',
                   ...
                   '--accept', '*.tif', f'{cn.CI95_mineral_soil_C_url}']
    uu.log_subprocess_output_full(cmd)
    ...
    uu.print_log(f"{soil_C_stdev_global} created.")
    ...
    if cn.count == 96:
        ...
        processes = 2
    uu.print_log(f"Creating mineral soil C stock stdev tiles with {processes} processors...")
    pool = multiprocessing.Pool(processes)
    pool.map(partial(uu.mp_warp_to_Hansen, source_raster=source_raster, out_pattern=out_pattern, dt=dt),
             tile_id_list)
    pool.close()
